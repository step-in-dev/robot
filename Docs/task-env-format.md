...
Change the subsection headings below (`operatorsLimit`, `customFunctionCallCount`, `ifLimit and whileLimit`, `requiredKeywords and bannedKeywords`) carefully: the website environment editor page (`editor.html` / `editor_ru.html`, generated by [`tools/build_website_content.py`](../tools/build_website_content.py)) links to them. If you rename or merge these headings, update `EDITOR_CONSTRAINT_DOC_ANCHORS` in [`tools/website_content_data.py`](../tools/website_content_data.py) and the checks in [`tests/test_build_website_content.py`](../tests/test_build_website_content.py). Keep the corresponding headings in the Russian translation aligned with the RU anchors.
