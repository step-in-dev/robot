...
- `robot/runtime_state.py` holds shared mutable execution state (active environment, debug session, command delay) and small helpers (`begin_solution_run` / `end_solution_run`, `active_robot`, `get_debug_session`, …) so executor/commands/debug code avoid ad-hoc global access.
...
- `robot/debug_runtime.py` implements debugger-aware `task()` behavior (trace/profile hooks, debug window coordination).
- `robot/runtime.py` is a thin facade: `task()`, compatibility re-exports, and script discovery; it wires loader, GUI, executor, and debug paths.
