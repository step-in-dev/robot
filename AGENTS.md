...
- Site-only community task discovery lives in `tools/site_catalog.py`, and explicit-path loading for community `.env` files lives in `tools/site_task_load.py`; these are used by the static website generator without changing `robot/loader.py`.
