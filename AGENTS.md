...
- `robot/loader.py` loads task definitions from `.env` files (JSON body with `envDtos` array and optional `todoText`, optional `operatorsLimit`, optional `customFunctionCallCount`), either from `ROBOT_TASKS_DIR` or the bundled `robot/tasks` directory.
