...
**Сайт:** [robot.stepindev.com](https://robot.stepindev.com) — каталог заданняў, даведнік каманд і артыкулы.
...
3. Захавайце файл з рашэннем побач з модулем `robot`. У якасці адпраўной кропкі выкарыстоўвайце файл **`sample_solution.py`** з архіва.
4. Каб запусціць іншае практыкаванне, змяніце радок, які перадаецца ў **`task()`** (гл. раздзел **Даступныя заданні для `task()`** вышэй).

Патрабаванні: **Python 3.7+** са стандартнай бібліятэкай (інтэрфейс выкарыстоўвае `tkinter`, які ўваходзіць у большасць усталёвак Python на настольных сістэмах).
