...
**ウェブサイト:** [robot.stepindev.com](https://robot.stepindev.com) — タスク一覧、コマンドリファレンス、記事。
...
3. 解答ファイルを `robot` モジュールの隣に保存します。アーカイブに含まれる **`sample_solution.py`** を出発点として使用してください。
4. 別の演習を実行するには、**`task()`** に渡す文字列を変更します（上記の **`task()` で利用可能な課題** のセクションを参照してください）。

必要な環境: **Python 3.7+** 標準ライブラリ（UI は `tkinter` を使用しており、ほとんどのデスクトップ用 Python インストールに含まれています）。
