...
Ниже три типичных этапа курса. Имена задач (`intro1`, `intro8`, `w2`) можно открыть в [каталоге задач](https://robot.stepindev.com/tasks/index_ru.html).

### Пример 1. Линейная программа — первые шаги

Задача: дойти из начальной клетки до цели простой цепочкой ходов, без условий и циклов.
...
Здесь уже видна **последовательность команд** как алгоритм. Повторяющиеся фрагменты у сильных учеников подводят к идее цикла «N раз» — ещё до формального введения `for`.
